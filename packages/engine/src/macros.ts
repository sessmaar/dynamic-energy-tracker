 ...
 *
 * Defaults reflect a moderate-protein cut: 2.0 g/kg, 25 % fat.
 */
export const computeMacroTargets = (
  dailyKcalTarget: number,
  bodyWeight: Kg,
  prefs: { proteinPerKg?: number; fatPct?: number } = {},
): MacroTargets => {
  const proteinPerKg = prefs.proteinPerKg ?? 2.0;
  const fatPct = prefs.fatPct ?? 0.25;
  if (fatPct < 0 || fatPct > 1) {
    throw new RangeError(`fatPct must be in [0,1], got ${fatPct}`);
  }
  const proteinG = Math.round(proteinPerKg * bodyWeight);
